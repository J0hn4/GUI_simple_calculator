...
photo = tkinter.PhotoImage(file="005-calculator.png")
...
def submit_number(number):
    ...
    #Insert the number into the end of the display
    display.insert('end', number)

    #if decimal was pressed, disable the decimal button so that it cannot be pressed twice
    if "." in display.get():
...
def operate(operator):
    ...
    #Delete the value of the first number form the entry display
...
def equal():
    """Run the stored oepration for two numbers"""
    if operation == 'add':
...
def negate():
    """Make value in display negative/positive"""
    value = display.get()
    new_value = -1 * int(value)
    display.delete(0, END)
...
def inverse():
    if display.get() == '0' or display.get() == '0.0':
        value = "ERROR"
...
exponent_button = tkinter.Button(button_frame, text='^', font =button_font, bg=light_green, command=lambda:operate('exponent'))
...
#Sixth Row
negate_button.grid(row=5, column=0, ipadx=20)
zero_button.grid(row=5, column=1, ipadx=20)
decimal_button.grid(row=5, column=2, ipadx=20)
equal_button.grid(row=5, column=3, ipadx=20)
